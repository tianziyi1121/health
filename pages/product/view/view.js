...
// 获取数据
  getData(id) {
    api.getProductCont({
      ...
      if(res.status == 1){
        res.data.pro_content = res.data.pro_content.replace(/<(?!\/?p|\/?IMG)[^<>]*>/ig, '')
          .replace(/<p([\s\w"=\/\.:;]+)((?:(style="[^"]+")))/ig, '<p')
          .replace(/<p>/ig, '<p class="f12" style="line-height: 26px;color: #666;">');
        const regex = new RegExp('<img', 'gi');
        res.data.pro_content = res.data.pro_content.replace(regex, `<img style="max-width: 345px;"`);
        this.setData({
          details: res.data
        })
  ...
  collect: function() {
    ...
    api.getMemberCollection(list).then((res) => {
      if (res.status == 1){
        this.setData({
